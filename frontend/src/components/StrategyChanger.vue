<template>
  ...
  >
    ...
    >
      <h2 class="text-2xl font-bold mb-4 text-gray-800">修改复习周期</h2>
      <p class="text-gray-600 mb-6">
        为 <span class="font-semibold">{{ chapterTitle }}</span> 修改复习周期
      </p>
      <p class="text-sm text-gray-500 mb-6">
        当前周期: <span class="font-semibold">{{ currentStrategy }}</span>
      </p>
      ...
      <div v-else class="space-y-3 mb-6">
        <button
          ...
          :disabled="currentStrategy === strategy.name"
        >
          <div class="flex items-start justify-between">
            ...
            <div
              v-if="currentStrategy === strategy.name"
              class="ml-3 flex-shrink-0 text-gray-400 text-xs"
            >
              当前
            </div>
            <div
              v-else-if="selectedStrategy === strategy.name"
...
const selectStrategy = (strategyName) => {
  if (strategyName !== props.currentStrategy) {
...
const confirm = async () => {
  if (
    ...
    selectedStrategy.value === props.currentStrategy
  )
    return;
